...
- Web UI группирует только возвращённые API `image_url` по `document_id`: один документ
...
| --- | --- | --- |
| Web UI | пространства, upload, polling, чат, визуальный туториал, источники | показывает безопасное сообщение API |
...
sequenceDiagram
  ...
  alt достаточно данных
    ...
    U->>U: image sources → группы по документам
    alt один визуальный документ
      U->>U: показать страницы по порядку
    else несколько визуальных документов
      U->>U: предложить выбрать туториал
    end
